...
public class EmployeeServiceImplimentation implements EmployeeService{
	...
	public Employee updateEmployeeById(EmployeeInputFormat employee, Integer employeeId) throws EmployeeException {
		Employee employeeWithId =  employeeRepository.findById(employeeId).orElseThrow(EmployeeException::new);
		...
		String department = employee.getDepartment().toString().toUpperCase();
		if(department.compareToIgnoreCase("Development") == 0)employeeWithId.setDepartment(Department.Development);
		else if(department.compareToIgnoreCase("QA") == 0)employeeWithId.setDepartment(Department.QA);
		else if(department.compareToIgnoreCase("Admin") == 0)employeeWithId.setDepartment(Department.Admin);
		else throw new EmployeeException("Incorrect department input");
		}
		
		if( employee.getDocuments() != null )employeeWithId.setDocuments(employee.getDocuments());
		...
		String gender = employee.getGender().toString().toUpperCase();
		if(gender.compareToIgnoreCase("MALE") == 0)employeeWithId.setGender(Gender.MALE);
		else if(gender.compareToIgnoreCase("FEMALE") == 0)employeeWithId.setGender(Gender.FEMALE);
		else throw new EmployeeException("Incorrect gender input");
		}
